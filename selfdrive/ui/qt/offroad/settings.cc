...
TogglesPanel::TogglesPanel(SettingsWindow *parent) : ListWidget(parent) {
  ...
  for (auto &[param, title, desc, icon] : toggle_defs) {
    ...
    if (param == "ExperimentalMode") {
      ...
      QObject::connect(toggle, &ToggleControl::toggleFlipped, [auto_toggle](bool enabled){
        auto_toggle->setVisible(enabled);
        if (!enabled) {
          // turn off AutoExperimentalMode if ExperimentalMode is turned off
          Params p;
          p.putBool("AutoExperimentalMode", false);
          auto_toggle->refresh();
        }
      });
...
void TogglesPanel::updateToggles() {
  ...
  if (!cp_bytes.empty()) {
    ...
    // Update visibility of AutoExperimentalMode toggle depending on ExperimentalMode state
    auto auto_exp_toggle_it = toggles.find("AutoExperimentalMode");
    if (auto_exp_toggle_it != toggles.end()) {
      bool exp_on = params.getBool("ExperimentalMode");
      auto_exp_toggle_it->second->setVisible(exp_on);
    }
  } else {
    experimental_mode_toggle->setDescription(e2e_description);
    auto auto_exp_toggle_it = toggles.find("AutoExperimentalMode");
